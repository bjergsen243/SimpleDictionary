...
public class DictionaryManagement extends Application {
  ...
  // set giong noi tieng Anh
    ...
    ggTranslate.setText("Google Translator");
    ggTranslate.setOnAction(
        event -> {
          try {
            useAPIButton();
          } catch (Exception e) {
    ...
    input
        .getChildren()
        .addAll(wordField, searchButton, speakButton, addAWord, deleteAWord, ggTranslate);
    ...
    // Create recommendList
        ...
        recommendWordsList = new ListView<>(observableList);
    ...
    if (!s.equals("")) {
      ...
      if (result > 0) {
        meaningLabel.setText(
            dictionary.getWords().get(result).getPhonetics()
                + "\n"
        ...
        meaningLabel.setText(
            "Not found,maybe you wrote it wrong,see recommend list beside or use API button");
  ...
  private void useAPIButton() throws Exception {
